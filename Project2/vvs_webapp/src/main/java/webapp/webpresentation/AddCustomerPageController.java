...
public class AddCustomerPageController extends PageController{
	...
	protected void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			String inputValidation = valid(vat,phone,designation);
			if (!inputValidation.equals("")) {
				throw new ApplicationException("data not valid:" + inputValidation);
			}
	...
	private String valid(String vat, String phone, String designation) {
		StringBuilder res = new StringBuilder();
		if (vat.equals("")) {
			res.append("vat is empty");
		}
		
		if (phone.equals("")) {
			res.append("/");
			res.append("phone is empty");
		}
		
		if (designation.equals("")) {
			res.append("/");
			res.append("designation is empty");
		}
		return res.toString();
	}
}
