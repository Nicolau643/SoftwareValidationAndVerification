		...
		try{
			...
			if(addr != null && sale != null) {
				if (isInt(sdh, sale, "Invalid Sale Id") && isInt(sdh, addr, "Invalid Address Id")) {
					...
					if (!cs.existAddress(addr_id)) {
						throw new ApplicationException("address id not exist: " + addr_id);
					}
			...
			if(isInt(ash, vat, "Invalid VAT number")) {
				...
				if (!cs.existVAT(vatNumber)) {
					throw new ApplicationException("VAT number not exist: " + vatNumber);
				}
