...
public class SaleRowDataGateway {
	...
	public SaleRowDataGateway(ResultSet rs) throws RecordNotFoundException {
		try {
			//fillAttributes(rs.getDate("date"), rs.getInt("customer_vat"));//falta aqui coisas!
			this.customerVat = rs.getInt("customer_vat");
			this.data = rs.getDate("date");
	...
	public void updateSale () throws PersistenceException {
		try (PreparedStatement statement = DataSource.INSTANCE.prepare(UPDATE_STATUS_SQL)){
			// set statement arguments
			System.out.println("aaaaa"+statusId);
	 ...
	 * The update customerPhone SQL statement
	 */
	/*private static final String	REMOVE_SALE_SQL =
			"delete from sale " +
					   "where customer_vat = ?";
	...
	public String toString() {
		return "SaleRowDataGateway [id=" + id + ", data=" + data + ", total=" + total + ", statusId=" + statusId
