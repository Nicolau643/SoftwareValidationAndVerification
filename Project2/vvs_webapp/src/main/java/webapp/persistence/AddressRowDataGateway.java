...
public class AddressRowDataGateway{
	...
	public void remove() throws PersistenceException {
		try (PreparedStatement statement = DataSource.INSTANCE.prepare(REMOVE_ADDRESS_BY_VAT)){
			...
			throw new PersistenceException("Internal error deleting address " + id + ".", e);
		}
	...
	private static final String	GET_ADDRESS_BY_ID =
			"select * from address " +
					   "where id = ?";
	public boolean existAddressById(int id) throws PersistenceException {
		try (PreparedStatement statement = DataSource.INSTANCE.prepare(GET_ADDRESS_BY_ID)){
			// set statement arguments
