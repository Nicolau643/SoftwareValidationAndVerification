...
public enum SaleService {
	...
	public void updateSale(int id) throws ApplicationException {
		try {
			SaleRowDataGateway sale = new SaleRowDataGateway().getSaleById(id); //TODO nao preenche os campos todos da SALE
	...
	public int addSaleDelivery(int sale_id, int addr_id) throws ApplicationException {
		try {
			...
			if (new SaleDeliveryRowDataGateway().getAllSaleDelivery(s.getCustomerVat()).stream().filter(sd -> sd.getSale_id()==sale_id).count() != 0) {
				throw new ApplicationException ("Already exist a sale with a address");
			}
	...
	/*public void removeSale(int vat) throws ApplicationException  {
		try {
			List<SaleRowDataGateway> s = new SaleRowDataGateway().getAllSales(vat);
			System.out.println(s.size());
			for (SaleRowDataGateway srd : s) {
				srd.removeSale();	
			}
		} catch (PersistenceException e) {
			System.out.println("aqui :(");
			throw new ApplicationException ("Can't get all sales.", e);
		}
