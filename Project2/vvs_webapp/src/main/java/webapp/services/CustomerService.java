...
public enum CustomerService {
	...
	public void addAddressToCustomer(int customerVat, String addr) throws ApplicationException {
		if (!isValidVAT (customerVat))
			...
			List<AddressRowDataGateway> vatAdresses = new AddressRowDataGateway().getCustomerAddresses(customerVat);
			
			for (AddressRowDataGateway adr : vatAdresses) {
				if (adr.getAddress().substring(0, address.getAddress().length()).equals(address.getAddress())) {
					System.out.println("entrou");
					throw new ApplicationException ("Can't add the address: (" + addr + ") to customer with vat number " + customerVat +  ": "
							+ "Duplicated Address.");
				}
			}
			address.insert();
	...
	public void removeCustomer(int vat) throws ApplicationException {
		if (!isValidVAT (vat))
			...
			CustomerRowDataGateway customer = new CustomerFinder().getCustomerByVATNumber(vat);
			//List<AddressRowDataGateway> customerAddresses = new AddressRowDataGateway().getCustomerAddresses(vat);
			//for (AddressRowDataGateway addr : customerAddresses) {
			//	addr.remove();
			//}
	...
	/**
	 * Verify if exist a customer with a certain vat number
	 * @param vatNumber
	 * @return
	 */
	public boolean existVAT(int vatNumber) {
		try {
			getCustomerByVat(vatNumber);
			return true;
	...
	/**
	 * Verify if exist a address with a certain id
	 * @param vatNumber
	 * @return
	 * @throws PersistenceException 
	 */
	public boolean existAddress(int addr_id)  {
		try {
			return new AddressRowDataGateway().existAddressById(addr_id);
		} catch (PersistenceException e) {
			return false;
		}
	}
