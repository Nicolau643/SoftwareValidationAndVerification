...
public class TST<T> {
    ...
    private Node<T> get(Node<T> x, String key, int d) {
        if (x == null) 
        	return null;
        if (key.length() == 0) 
        	throw new IllegalArgumentException("key must have length >= 1");  // nunca entrará aqui porque esta verif ja foi feita
        char c = key.charAt(d);
    ...
    public String longestPrefixOf(String query) {
        ...
        Node<T> x = root; //I6
        int i = 0; //I7
        while (x != null && i < query.length()) { //I8 //I9
            char c = query.charAt(i); //I10
            if      (c < x.c) x = x.left; //I11 I12
            else if (c > x.c) x = x.right; //I13 I14
            else {
                i++; //I15
                if (x.val != null) //I16
                	length = i; //I17
	...
	public boolean equals(Object obj) {
		...
		Iterator<String> s = this.keys().iterator();//I10
		String next; //I11
		while (s.hasNext()) {//I12
			next = s.next();//I13
			if (!other.contains(next) || !other.get(next).equals(get(next))) { //I14 I15
				return false; //I16
			}
	...
	/**
	 * Delete a content using a key (these function has memory problems but if we make many puts and deletes,
	 *  it is faster because there is no need to create new nodes, just add the data T)
	 * @param key - key to delete in the TST 
	 */
	public void delete(String key) {
		
		if (key == null || key.length() == 0) { // I1 I2
			throw new IllegalArgumentException(); // I3
		}
		
		if (!contains(key)) { //I4
			throw new IllegalArgumentException("Nao existe para apagar"); //I5
		}
	...
	private void deleteAux(Node<T> node, String data, int i) {
		
		char c = data.charAt(i);
		
		
		if (c < node.c) {
			deleteAux(node.left,data,i);
		}else if(c > node.c) {
			deleteAux(node.right,data,i);
		}else if(i == data.length()-1){
			node.val = null;
		} else {
			deleteAux(node.mid,data,i+1);
		}
